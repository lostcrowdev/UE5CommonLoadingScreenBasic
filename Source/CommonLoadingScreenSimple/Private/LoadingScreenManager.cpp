...
void ULoadingScreenManager::UpdateLoadingScreen()
{
	...
	if (bLogLoadingScreenStatus || DebugReasonForShowingOrHidingLoadingScreen != LastDebugReasonLogged)
	{
		LastDebugReasonLogged = DebugReasonForShowingOrHidingLoadingScreen;
		UE_LOG(LogLoadingScreen, Log, TEXT("[%s] %s"), bCurrentlyShowingLoadingScreen ? TEXT("SHOWN") : TEXT("HIDDEN"), *DebugReasonForShowingOrHidingLoadingScreen);
	}
}
...
void ULoadingScreenManager::ShowLoadingScreen()
{
	...
	{
		UE_LOG(LogLoadingScreen, Log, TEXT("[SHOW] %s"), *DebugReasonForShowingOrHidingLoadingScreen);
	}
	else
	{
		UE_LOG(LogLoadingScreen, Log, TEXT("[SHOW] %s"), *DebugReasonForShowingOrHidingLoadingScreen);
...
void ULoadingScreenManager::HideLoadingScreen()
{
	...
	if (IsShowingInitialLoadingScreen())
	{
		UE_LOG(LogLoadingScreen, Log, TEXT("[HIDE] %s"), *DebugReasonForShowingOrHidingLoadingScreen);
	}
	else
	{
		UE_LOG(LogLoadingScreen, Log, TEXT("[HIDE] %s"), *DebugReasonForShowingOrHidingLoadingScreen);

		UE_LOG(LogLoadingScreen, Log, TEXT("[HIDE] Garbage collecting before dropping load screen"));
	...
	UE_LOG(LogLoadingScreen, Log, TEXT("[HIDE] Visible for %.2fs"), LoadingScreenDuration);
